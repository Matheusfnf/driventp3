...
describe("GET /payments", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 401 when user doesnt own given ticket", async () => {
      ...
      const token = await generateValidToken(user);
      await createEnrollmentWithAddress(user);
      const ticketType = await TicketTypeCreated();

      const otherUser = await createUser();
      const otherUserEnrollment = await createEnrollmentWithAddress(otherUser);
      const ticket = await TicketCreated(otherUserEnrollment.id, ticketType.id, TicketStatus.RESERVED);
    ...
    it("should respond with status 200 and with payment data", async () => {
      ...
      const ticketType = await TicketTypeCreated();
      const ticket = await TicketCreated(enrollment.id, ticketType.id, TicketStatus.RESERVED);
