...
import { createEnrollmentWithAddress, TicketCreated, TicketTypeCreated, createUser } from "../factories";
...
describe("GET /hotels", () => {
  it("should respond with status 401 if token is not given in request", async () => {
    const response = await server.get("/hotels");

    expect(response.status).toBe(httpStatus.UNAUTHORIZED);
  });

  it("should respond with status 401 if token given isnt valid in request", async () => {
  ...
  it("should respond with status 401 if there is no session for given token", async () => {
    const noSessionUser = await createUser();
    const token = jwt.sign({ userId: noSessionUser.id }, process.env.JWT_SECRET);
  ...
  describe("when token is valid", () => {
    it("should respond with status 404 when havent enrollment for user", async () => {
    ...
    it("should respond with status 402 when there is no paid ticket for given user", async () => {
      ...
      const ticketType = await TicketTypeCreated(false, true);
      const enrollment = await createEnrollmentWithAddress(user);
      const token = await generateValidToken(user);
      await TicketCreated(enrollment.id, ticketType.id, TicketStatus.RESERVED);
    ...
    it("should respond with status 402 when there is remote ticket for given user", async () => {
      ...
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await TicketTypeCreated(true);
      await TicketCreated(enrollment.id, ticketType.id, TicketStatus.PAID);
    ...
    it("should respond with status 402 when there is no includes hotels ticket for given user", async () => {
      ...
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await TicketTypeCreated(false, false);
      await TicketCreated(enrollment.id, ticketType.id, TicketStatus.RESERVED);
    ...
    it("should respond with status 200 if data and ticket", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await TicketTypeCreated(false, true);
...
describe("GET /hotels/:hotelId", () => {
  ...
  it("should respond with status 401 if doesnt have session for that user", async () => {
  ...
  describe("when token is valid", () => {
    it("should respond with status 404 when no enrollment for given user", async () => {
      const token = await generateValidToken();
